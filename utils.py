...
class ReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        for i in idxes:
            ...
            dones.append(done)
        return np.array(obses_t).astype(np.float32) / 255.0, np.array(actions), np.array(rewards), np.array(obses_tp1).astype(np.float32) / 255.0, np.array(dones)
...
class LazyFrames(object):
    ...
    def _force(self):
        if self._out is None:
            ...
            self._out = self._out.transpose(2, 0, 1) # frames x h x w
            self._frames = None
        return self._out
...
def make_atari(env_id, max_episode_steps=None, frameskip=4):
    env = gym.make(env_id)
...
def wrap_deepmind(env, episode_life=True, clip_rewards=True, frame_stack=False, scale=False, stacks=4):
